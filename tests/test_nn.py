...
def test_avg(t: Tensor) -> None:
    ...
    out = minitorch.avgpool2d(tm, (2, 2))
    assert_close(
        out[0, 0, 0, 0], sum([tm[0, 0, i, j] for i in range(2) for j in range(2)]) / 4.0
    )

    #out = minitorch.avgpool2d(tm, (2, 1))
    #assert_close(
    #    out[0, 0, 0, 0], sum([tm[0, 0, i, j] for i in range(2) for j in range(1)]) / 2.0
    #)

    #out = minitorch.avgpool2d(tm, (1, 2))
    #assert_close(
    #    out[0, 0, 0, 0], sum([tm[0, 0, i, j] for i in range(1) for j in range(2)]) / 2.0
    #)
    #minitorch.grad_check(lambda t: minitorch.avgpool2d(tm, (2, 2)), t)
